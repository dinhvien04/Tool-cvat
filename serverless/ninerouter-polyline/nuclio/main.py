"""Nuclio Function Entry Point for 9Router Polyline CVAT Detector.

Receives CVAT inference requests, forwards images to local 9Router vision models,
and returns CVAT shapes routed according to Policy C:
...
def init_context(context):
    ...
    context.logger.info("Initializing 9Router Polyline Nuclio function context...")
    try:
        handler_instance = ModelHandler()
        context.user_data.model_handler = handler_instance
        context.logger.info("9Router Polyline context initialized successfully.")
    except Exception as e:
        context.logger.error(f"Failed to initialize 9Router Polyline context: {e}")
        raise


def handler(context, event):
    """Handle incoming detector event from CVAT."""
    context.logger.info("Handling CVAT Polyline detector request...")
    ...
    try:
        ...
        context.logger.error(f"Image or validation error during Polyline inference: {e}")
        return context.Response(
        ...
        context.logger.error(f"Error during 9Router Polyline inference: {err_msg}")
        return context.Response(
