"""Nuclio Function Entry Point for 9Router Vision Box+Mask CVAT Detector.

Receives CVAT inference requests, forwards images to local 9Router vision models,
and returns BOTH CVAT rectangle shapes and native mask shapes with shared group_id.
"""
...
def init_context(context):
    ...
    context.logger.info("Initializing 9Router Vision Box+Mask Nuclio function context...")
    try:
        handler_instance = ModelHandler()
        context.user_data.model_handler = handler_instance
        context.logger.info("9Router Vision Box+Mask context initialized successfully.")
    except Exception as e:
        context.logger.error(f"Failed to initialize 9Router Vision Box+Mask context: {e}")
        raise


def handler(context, event):
    """Handle incoming detector event from CVAT."""
    context.logger.info("Handling CVAT Box+Mask detector request...")
    ...
    try:
        ...
        context.logger.error(f"Image or validation error during Box+Mask inference: {e}")
        return context.Response(
        ...
        context.logger.error(f"Error during 9Router vision Box+Mask inference: {err_msg}")
        return context.Response(
