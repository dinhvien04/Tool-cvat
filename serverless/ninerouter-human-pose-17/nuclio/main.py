"""Nuclio Function Entry Point for 9Router Human Pose 17 CVAT Detector.

Receives CVAT inference requests, forwards images to local 9Router vision models,
and returns CVAT native skeleton shapes according to the COCO 17-Keypoint topology:
- Exactly 17 canonical keypoints
- Native CVAT skeleton shape type ("skeleton")
- Points sublabels with visibility and occlusion mapping
"""
...
def init_context(context):
    ...
    context.logger.info("Initializing 9Router Human Pose 17 Nuclio function context...")
    try:
        handler_instance = ModelHandler()
        context.user_data.model_handler = handler_instance
        context.logger.info("9Router Human Pose 17 context initialized successfully.")
    except Exception as e:
        context.logger.error(f"Failed to initialize 9Router Human Pose 17 context: {e}")
        raise


def handler(context, event):
    """Handle incoming detector event from CVAT."""
    context.logger.info("Handling CVAT Human Pose 17 detector request...")
    ...
    try:
        ...
        context.logger.error(f"Image or validation error during Human Pose 17 inference: {e}")
        return context.Response(
        ...
        context.logger.error(f"Error during 9Router Human Pose 17 inference: {err_msg}")
        return context.Response(
