"""ModelHandler for 9Router Rectangle + Mask Nuclio Detector.

Bridges CVAT's serverless detector invocation with the local 9Router vision model.
Enforces Policy A:
- Exactly 14 foreground instance labels
- Emits paired rectangle + mask sharing identical integer group_id
"""
...
from core.vision_contract import MODE_RECTANGLE_MASK

logger = logging.getLogger("cvat.nuclio.ninerouter.rectangle_mask")
...
def load_spec_labels_from_function_yaml(yaml_path: Optional[Path | str] = None) -> List[str]:
    ...
    try:
        ...
        logger.warning(f"Could not parse labels from {target_path}: {e}. Using default BOX_MASK_LABELS.")

    return list(BOX_MASK_LABELS)


class ModelHandler:
    """Manages the 9Router vision client and runs Policy A (Rectangle + Mask) inference for CVAT."""
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"ModelHandler(rectangle_mask, base_url={self.base_url!r}, "
    ...
    def infer(
        ...
        """Process incoming image bytes and return Policy A CVAT shapes (paired rectangle + mask).

        Args:
            image_bytes: Raw JPEG/PNG image binary data.
            threshold: Minimum confidence score [0.0, 1.0].
            mode: Optional detection mode override. Defaults to self.default_mode ('rectangle_mask').
            roi: Optional sub-region of interest [x1, y1, x2, y2] to crop and annotate.

        Returns:
            List of CVAT shape dictionaries routed according to Policy A (paired rectangle + mask).
        """
        if not image_bytes:
        ...
        logger.info(
            f"Rectangle + Mask inference complete: model={self.active_model}, mode={active_mode}, "
