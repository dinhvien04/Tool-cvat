"""Nuclio Function Entry Point for 9Router Face VF50 CVAT Detector.

Receives CVAT inference requests, forwards images to local 9Router vision models,
and returns CVAT native skeleton shapes according to VinAI VF50 Landmark topology:
...
def init_context(context):
    ...
    context.logger.info("Initializing 9Router Face VF50 Nuclio function context...")
    try:
        handler_instance = ModelHandler()
        context.user_data.model_handler = handler_instance
        context.logger.info("9Router Face VF50 context initialized successfully.")
    except Exception as e:
        context.logger.error(f"Failed to initialize 9Router Face VF50 context: {e}")
        raise


def handler(context, event):
    """Handle incoming detector event from CVAT."""
    context.logger.info("Handling CVAT Face VF50 detector request...")
    ...
    try:
        ...
        context.logger.error(f"Image or validation error during Face VF50 inference: {e}")
        return context.Response(
        ...
        context.logger.error(f"Error during 9Router Face VF50 inference: {err_msg}")
        return context.Response(
