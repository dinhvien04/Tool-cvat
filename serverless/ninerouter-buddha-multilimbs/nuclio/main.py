"""Nuclio Function Entry Point for 9Router Buddha Multi-Limb Pose CVAT Detector.

Receives CVAT inference requests, forwards images to local 9Router vision models,
and returns CVAT native skeleton shapes according to the Buddha Multi-Limb topology:
- Central Body: 17-Keypoint Pose17 skeleton ('person')
- Central Face: 50-Landmark VF50 7 component skeletons ('longmaytrai' .. 'moitrong')
- Dynamic Buddha Arms: 3-joint skeletons ('buddha_arm': root, elbow, wrist)
- Dynamic Buddha Hands: 21-joint skeletons ('buddha_hand': wrist + 5 finger chains)
"""
...
def init_context(context):
    ...
    context.logger.info("Initializing 9Router Buddha Multi-Limb Nuclio function context...")
    try:
        handler_instance = ModelHandler()
        context.user_data.model_handler = handler_instance
        context.logger.info("9Router Buddha Multi-Limb context initialized successfully.")
    except Exception as e:
        context.logger.error(f"Failed to initialize 9Router Buddha Multi-Limb context: {e}")
        raise


def handler(context, event):
    """Handle incoming detector event from CVAT."""
    context.logger.info("Handling CVAT Buddha Multi-Limb detector request...")
    ...
    if event_type:
        ...
        sig_header = None
        try:
            from app.cvat_sync import extract_cvat_signature_header
    ...
    try:
        ...
        context.logger.error(f"Image or validation error during Buddha Multi-Limb inference: {e}")
        return context.Response(
        ...
        context.logger.error(f"Error during 9Router Buddha Multi-Limb inference: {err_msg}")
        return context.Response(
