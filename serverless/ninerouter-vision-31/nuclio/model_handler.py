"""ModelHandler for 9Router Vision 31-Label Multi-Shape Nuclio Detector.

Bridges CVAT's serverless detector invocation with the local 9Router vision model.
Routes all 31 road-scene labels into their appropriate CVAT shape geometries:
- Instance Objects (14 labels): Bounding Box + Paired Instance Mask (shared group_id)
- Semantic Regions (10 labels): Semantic Mask with contour points (ungrouped)
- Lane Markings (7 labels): Thin Ribbon Mask / Polyline / Polygon (ungrouped)
"""
...
def load_spec_labels_from_function_yaml(yaml_path: Optional[Path | str] = None) -> List[str]:
    ...
    try:
        ...
        logger.warning(f"Could not parse labels from {target_path}: {e}. Using default 31 labels.")

    return list(ALL_31_LABELS)


class ModelHandler:
    """Manages the 9Router vision client and runs unified 31-label multi-shape inference for CVAT."""
    ...
    def __init__(
        ...
        env_mode = os.getenv("DETECTION_MODE", MODE_FULL_31)
        self.default_mode = (mode or env_mode or MODE_FULL_31).strip().lower()
        if self.default_mode not in (MODE_BOX, MODE_MASK, MODE_BOX_AND_MASK, MODE_FULL_31):
            self.default_mode = MODE_FULL_31
        ...
        logger.info(
            f"Initialized ModelHandler (Full 31 Labels): base_url={self.base_url!r}, "
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"ModelHandler(full_31, base_url={self.base_url!r}, "
    ...
    def infer(
        ...
        """Process incoming image bytes and return full 31-label CVAT shapes.

        Args:
            image_bytes: Raw JPEG/PNG image binary data.
            threshold: Minimum confidence score [0.0, 1.0].
            mode: Optional detection mode override ('full_31', 'box_and_mask', 'box', 'mask').
        ...
        Returns:
            List of CVAT shape dictionaries routed according to taxonomy:
            - Instances: rectangle + mask with shared group_id
            - Regions: mask with contour points (group_id = None)
            - Lanes: polyline, polygon, or mask (group_id = None)
        """
        if not image_bytes:
            raise ValueError("Empty image payload received")

        active_mode = (mode or self.default_mode).strip().lower()
        if active_mode not in (MODE_BOX, MODE_MASK, MODE_BOX_AND_MASK, MODE_FULL_31):
            raise ValueError(
                f"Invalid detection mode {active_mode!r}. Must be one of: "
                f"'{MODE_BOX}', '{MODE_MASK}', '{MODE_BOX_AND_MASK}', '{MODE_FULL_31}'"
            )
        ...
        logger.info(
            f"Full 31 inference complete: model={self.active_model}, mode={active_mode}, "
